...
We will see a quick setup to configure the ``firefly`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``firefly`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``firefly`` formulas under ``/srv/salt/firefly/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/firefly/parameters/dns:domain/
    ln -s dns:domain /srv/salt/firefly/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/firefly/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/firefly/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``firefly`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``firefly`` formulas under ``/srv/salt/firefly/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/firefly/parameters/roles
...
We create a configuration for the role ``firefly/server`` in ``/srv/salt/firefly/parameters/roles/firefly/server.yaml``:
...
We create another configuration for the role ``firefly/client`` in ``/srv/salt/firefly/parameters/roles/firefly/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles firefly/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles firefly/client
...
.. code-block:: console
    ...
    mv /srv/salt/firefly/parameters/dns:domain/  '/srv/salt/firefly/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-firefly-as-json:
      test.nop:
        - name: {{ firefly | json }}
...
Here is an example based on `template-formula/firefly/config/file.sls`_:
    ...
    firefly-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='firefly-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ firefly.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``firefly`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ firefly['winner'] }}
...
.. _template-formula/firefly/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/firefly/config/file.sls
